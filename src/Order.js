...
function Order() {
  ...
  useEffect(() => {
    ...
    if (selectedRecipe) {
      ...
      setTotalPrice({ subtotal: subtotal.toFixed(2), discount: discount.toFixed(2), tax: tax.toFixed(2), final: total.toFixed(2) });

      if (selectedPackage) {
        const containerSize = parseFloat(selectedPackage.Size);
        setPackageError(containerSize && form.pounds % containerSize !== 0
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ phone: "", name: "", email: "", address: "", recipe: "", pounds: 1, packaging: packagingOptions[0]?.Type || "", coupon: "" });
        setTotalPrice({ subtotal: "0.00", discount: "0.00", tax: "0.00", final: "0.00" });
        setErrors({});
        setPackageError("");
  ...
  return (
    <section style={{ maxWidth: "600px", margin: "40px auto", padding: "20px", backgroundColor: "#fff", borderRadius: "12px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", fontFamily: "Arial, sans-serif" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
        {["phone", "name", "email", "address"].map(f => (
          <label key={f}>
            {f.charAt(0).toUpperCase() + f.slice(1)}:
            <input type={f === "email" ? "email" : "text"} name={f} value={form[f]} onChange={handleChange} style={inputStyle} required />
            {errors[f] && <div style={errorStyle}>{errors[f]}</div>}
          </label>
        ))}
        <label>
          Select Recipe:
        ...
        <label>
          ...
          <select name="packaging" value={form.packaging} onChange={handleChange} style={selectStyle} required>
            {packagingOptions.length === 0 ? <option value="">Loading packages...</option> : packagingOptions.map((p, idx) => <option key={idx} value={p.Type}>{p.Type} - {parseFloat(p.Size)} lb per container</option>)}
          </select>
